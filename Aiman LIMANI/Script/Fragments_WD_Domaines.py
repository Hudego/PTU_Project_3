...
# Fonction pour découper en fragments selon pLDDT avec un minimum de 9 résidus consécutifs au-dessus du seuil de 70
def extract_fragments_by_disorder(model, plddt_scores, threshold=70, min_consecutive=9, tolerance=5):
    fragments = []
    fragment = []
    consecutive_count = 0
    low_score_count = 0
    ...
    for i, residue in enumerate(model.get_residues()):
        if plddt_scores[i] >= threshold:
            ...
            # Vérifier si la taille minimale pour commencer un fragment est atteinte
            if consecutive_count >= min_consecutive and len(fragment) == consecutive_count:
                # Valider le fragment s'il atteint la longueur minimale après les acides aminés consécutifs
            ...
            # Permettre une tolérance jusqu'à un certain nombre d'acides aminés en-dessous du seuil
            if low_score_count <= tolerance and fragment:
                fragment.append(residue)
            else:
                # Finir le fragment si la tolérance est dépassée
                if len(fragment) >= min_consecutive:
...
for uniprot_id in df['code']:
    ...
    if pdb_file:
        ...
        fragments = extract_fragments_by_disorder(model, plddt_scores, threshold=70, min_consecutive=9)
